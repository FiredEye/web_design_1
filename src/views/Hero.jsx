...
const Hero = () => {
  ...
  return (<div  className="h-screen relative flex flex-col items-center" style={{background:`url(${heroImage})`,backgroundPosition:"bottom",backgroundRepeat:"no-repeat",backgroundSize:"cover"}}>
...
<img src={cartIcon} className="hidden md:block cursor-pointer" alt="add-to-cart" />
<HiMenuAlt3 size={30} className="block md:hidden cursor-pointer text-white" onClick={()=>setShowMobileMenu(prev=>!prev)}/>
<div className={`block md:hiden w-full fixed ${!showMobileMenu?"-top-[410px]":"top-0"} left-0 bg-[#dde0e5] h-[410px] transition-all duration-[800ms] shadow-xl z-10 py-8 px-12 rounded-b-xl`}>
  ...
  <NavLink to='care'mobileMenu>Care</NavLink>  
</ul><img src={cartIcon} className="mt-8 mx-auto cursor-pointer" alt="add-to-cart"/>
</div>
    </div>
    <FadeIn delay={0.2} direction='down' padding fullwidth>
      <h1 className="mt-[50px] text-center text-5xl leading-tight xs:text-[60px] text-white max-w-[1050px]">{heroTitle}</h1>
    </FadeIn>
    ...
    <FadeIn delay={0.2} direction="up" padding fullWidth>
      <div className="relative w-full xs:w-[460px] mt-11 mx-auto">
        ...
        <img src={searchIcon} className="absolute top-2/4 -translate-y-2/4 right-3 h-11 w-11 cursor-pointer" alt="Search icon" />
      </div>
    </FadeIn>
